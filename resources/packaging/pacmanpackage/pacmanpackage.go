// Package pacmanpackage manages packages under pacman.
//
// The kind is its own even though the handler is shared: a manifest saying
// PacmanPackage cannot be mistaken for one that would work on another distribution,
...
func New(p *provider.Provider) core.Handler {
	return pkgkind.New(p, "PacmanPackage", "pacmanpackages", "pacman",
		"A package managed by Arch's pacman.",
		"pacman")
}
