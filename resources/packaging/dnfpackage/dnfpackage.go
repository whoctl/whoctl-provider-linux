// Package dnfpackage manages packages under dnf.
//
// The kind is its own even though the handler is shared: a manifest saying
// DnfPackage cannot be mistaken for one that would work on another distribution,
...
func New(p *provider.Provider) core.Handler {
	return pkgkind.New(p, "DnfPackage", "dnfpackages", "dnf",
		"A package managed by dnf or yum on the Red Hat family.",
		"dnf", "yum", "rpm")
}
